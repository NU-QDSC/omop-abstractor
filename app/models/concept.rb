class Concept < ApplicationRecord
  ...
  DOMAIN_ID_UNIT = 'Unit'
  DOMAIN_ID_VISIT = 'Visit'
  DOMAIN_IDS = [DOMAIN_ID_CONDITION, DOMAIN_ID_GENDER, DOMAIN_ID_ETHNICITY, DOMAIN_ID_MEASUREMENT, DOMAIN_ID_METADATA, DOMAIN_ID_OBSERVATION, DOMAIN_ID_PROCEDURE, DOMAIN_ID_PROVIDER, DOMAIN_ID_RACE, DOMAIN_ID_RELATIONSHIP, DOMAIN_ID_ROUTE, DOMAIN_ID_TYPE_CONCEPT, DOMAIN_ID_UNIT, DOMAIN_ID_VISIT]
  ...
  CONCEPT_CLASS_PHYSICIAN_SPECIALITY = 'Physician Specialty'
  ...
  def self.visit_concepts
    standard.valid.where(domain_id: Concept::DOMAIN_ID_VISIT)
  end

  def self.visit_types
    valid.where(domain_id: Concept::DOMAIN_ID_TYPE_CONCEPT, vocabulary_id: Concept::VOCABULARY_ID_VISIT_TYPE, concept_class_id: Concept::CONCEPT_CLASS_VISIT_TYPE)
  end

  def self.provider_specialties
    specialities = [
     38004459, #Neurosurgery
     45756790, #Neuropathology
     38004473  #Thoracic Surgery
    ]
    valid.where('domain_id = ? AND concept_id IN(?)', Concept::DOMAIN_ID_PROVIDER, specialities)
  end
  ...
  def self.search(search_token)
    ...
    search_tokens.each do |st|
      all_concepts << "lower(concept_name) like '%#{st.downcase}%'"
    end
    where(all_concepts.join(' OR '))
  end
end