...
require 'csv'
namespace :chawla_pancreas do
  desc 'Load schemas CLAMP Chawla Pancreas'
  task(schemas_clamp_chawla_pancreas: :environment) do |t, args|
    ...
    pancreas_histologies = Icdo3Histology.by_primary_pancreas
    pancreas_histologies.each do |histology|
    ...
    # abstractor_object_value = abstractor_abstraction_schema.abstractor_object_values.where(vocabulary_code: '8140/2').first
    # abstractor_object_value.favor_more_specific = true
    # abstractor_object_value.save!
    #
    # abstractor_object_value = abstractor_abstraction_schema.abstractor_object_values.where(vocabulary_code: '8140/3').first
    # abstractor_object_value.favor_more_specific = true
    # abstractor_object_value.save!
    #
    ...
    pancreas_sites = Icdo3Site.by_primary_pancreas
    pancreas_sites.each do |site|
    ...
    abstractor_object_value = abstractor_abstraction_schema.abstractor_object_values.where(vocabulary_code: 'C25.9').first
    abstractor_object_value.favor_more_specific = true
    abstractor_object_value.save!
    ...
    Abstractor::AbstractorAbstractionSchemaPredicateVariant.where(abstractor_abstraction_schema: abstractor_abstraction_schema, value: 'prior history of').first_or_create

    sites = Icdo3Site.by_primary_metastatic_pancreas
    sites.each do |site|
    ...
    Abstractor::AbstractorAbstractionSchemaPredicateVariant.where(abstractor_abstraction_schema: abstractor_abstraction_schema, value: 'lvi').first_or_create
    ...
    Abstractor::AbstractorAbstractionSchemaPredicateVariant.where(abstractor_abstraction_schema: abstractor_abstraction_schema, value: 'number of positive').first_or_create
    ...
    Abstractor::AbstractorAbstractionSourceSection.where( abstractor_abstraction_source: abstractor_abstraction_source, abstractor_section: abstractor_section_specimen).first_or_create
    Abstractor::AbstractorSubjectGroupMember.where(:abstractor_subject => abstractor_subject, :abstractor_subject_group => primary_cancer_group, :display_order => 6).first_or_create
    #End surgery date
    ...
    abstractor_subject = Abstractor::AbstractorSubject.where(:subject_type => 'NoteStableIdentifier', :abstractor_abstraction_schema => abstractor_abstraction_schema, namespace_type: Abstractor::AbstractorNamespace.to_s, namespace_id: abstractor_namespace_outside_surgical_pathology.id).create
    abstractor_abstraction_source = Abstractor::AbstractorAbstractionSource.where(abstractor_subject: abstractor_subject, from_method: 'note_text', :abstractor_rule_type => name_value_rule, abstractor_abstraction_source_type: source_type_custom_nlp_suggestion, custom_nlp_provider: 'custom_nlp_provider_clamp').first_or_create
    Abstractor::AbstractorAbstractionSourceSection.where( abstractor_abstraction_source: abstractor_abstraction_source, abstractor_section: abstractor_section_specimen).first_or_create
    Abstractor::AbstractorSubjectGroupMember.where(:abstractor_subject => abstractor_subject, :abstractor_subject_group => metastatic_cancer_group, :display_order => 6).first_or_create
    #End surgery date
    #End metastatic
  ...
  desc "Clamp Dictionary Chawla Pancreas"
  task(clamp_dictionary_chawla_pancreas: :environment) do  |t, args|
    dictionary_items = []
    ...
    predicates << 'has_perineural_invasion'
    ...
    File.open 'lib/setup/data_out/abstractor_clamp_chawla_pancreas_data_dictionary.txt', 'w' do |f|
  ...
  desc "Load Chawla Pancreas data into the batch table"
  task(chawla_pancreas_batch_data: :environment) do |t, args|
    files = ['lib/setup/data/chawla_Pancreas/Specimen Collection Facility Pathology Cases with Surgeries 1.xlsx']
  ...
  desc "Load Biorepository Chawla Pancreas data"
  task(chawla_pancreas_data: :environment) do |t, args|
    files = ['lib/setup/data/chawla_pancreas/Specimen Collection Facility Pathology Cases with Surgeries 1.xlsx']
