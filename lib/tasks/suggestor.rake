...
namespace :suggestor do
  ...
  task(do_multiple_aml: :environment) do |t, args|
    abstractor_suggestor_aml(multiple: true)
  end

  task(do_multiple_aml_sufficiency: :environment) do |t, args|
...
def abstractor_suggestor_aml(options = {})
  options.reverse_merge!({ multiple: false })

  Abstractor::AbstractorNamespace.where(name: ['Outside Diagnostic Pathology']).all.each do |abstractor_namespace|
    puts 'here is the namespace'
    puts abstractor_namespace.name
    #All
    abstractable_events = abstractor_namespace.subject_type.constantize.missing_abstractor_namespace_event(abstractor_namespace.id).joins(abstractor_namespace.joins_clause).where(abstractor_namespace.where_clause).order('note.person_id ASC, note.note_date ASC')
    ...
    abstractable_events.each_with_index do |abstractable_event, i|
      ...
      # child_pid = fork do
        if options[:multiple]
          ...
          note_titles = ['Addendum', 'Comment', 'Amended']
          procedure_occurrence_options = {}
          procedure_occurrence_options[:username] = 'mjg994'
          procedure_occurrence_options[:include_parent_procedures] = false
          note = abstractable_event.note
  ...
  Abstractor::AbstractorNamespace.where(name: 'Diagnostic Pathology').all.each do |abstractor_namespace|
    puts 'here is the namespace'
    puts abstractor_namespace.name
    #All
    abstractable_events = abstractor_namespace.subject_type.constantize.missing_abstractor_namespace_event(abstractor_namespace.id).joins(abstractor_namespace.joins_clause).where(abstractor_namespace.where_clause).order('note.person_id ASC, note.note_date ASC')
...
def abstractor_suggestor_aml_sufficiency(options = {})
  options.reverse_merge!({ multiple: false })

  Abstractor::AbstractorNamespace.where(name: ['Outside Diagnostic Pathology Sufficiency']).all.each do |abstractor_namespace|
    puts 'here is the namespace'
    puts abstractor_namespace.name
    #All
    abstractable_events = abstractor_namespace.subject_type.constantize.missing_abstractor_namespace_event(abstractor_namespace.id).joins(abstractor_namespace.joins_clause).where(abstractor_namespace.where_clause).order('note.person_id ASC, note.note_date ASC')
  ...
  Abstractor::AbstractorNamespace.where(name: 'Diagnostic Pathology Sufficiency').all.each do |abstractor_namespace|
    puts 'here is the namespace'
    puts abstractor_namespace.name
    #All
    abstractable_events = abstractor_namespace.subject_type.constantize.missing_abstractor_namespace_event(abstractor_namespace.id).joins(abstractor_namespace.joins_clause).where(abstractor_namespace.where_clause).order('note.person_id ASC, note.note_date ASC')
...
# abstractable_events = abstractor_namespace.subject_type.constantize.missing_abstractor_namespace_event(abstractor_namespace.id).joins(abstractor_namespace.joins_clause).where(abstractor_namespace.where_clause).order('note.person_id ASC, note.note_date ASC').to_sql