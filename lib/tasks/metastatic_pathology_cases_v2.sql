...
(
  ...
  has_cancer_histology_suggestion_match_values         text NULL,
  ...
  has_cancer_site_suggestion_sentences text NULL,

  has_metastatic_cancer_primary_site       varchar(255)  NULL,
  has_metastatic_cancer_primary_site_suggestions              text NULL,
  has_metastatic_cancer_primary_site_suggestion_sentences text NULL,
...
INSERT INTO cancer_diagnosis_abstractions (
 ...
 , has_cancer_histology_suggestion_match_values
 , has_cancer_histology_suggestion_sentences
 , has_cancer_histology_negated

 , has_cancer_site
 , has_cancer_site_suggestions
 , has_cancer_site_suggestion_sentences

 , has_metastatic_cancer_primary_site
 , has_metastatic_cancer_primary_site_suggestions
 , has_metastatic_cancer_primary_site_suggestion_sentences
...
SELECT note_stable_identifier.id
     ...
     , array_to_string(array(
       ...
       , array_to_string(array(
           SELECT DISTINCT ass.match_value
           FROM abstractor_abstractions aa2 JOIN abstractor_suggestions asg2 on aa2.id = asg2.abstractor_abstraction_id
                                            JOIN abstractor_subjects asb2 ON aa2.abstractor_subject_id = asb2.id
                                            JOIN abstractor_abstraction_schemas aas2 ON asb2.abstractor_abstraction_schema_id = aas2.id
                                            JOIN abstractor_abstraction_group_members aagm ON aa2.id = aagm.abstractor_abstraction_id
           ...
           AND asg2.system_rejected = false
           AND asg.name = 'Metastatic Cancer'
         ), '|') AS has_cancer_histology_suggestion_match_values
       , array_to_string(array(
           ...
           AND asg2.system_rejected = false
           AND asg.name = 'Metastatic Cancer'
           AND lower(ass.sentence_match_value) NOT LIKE '%tumor-nodes-metastasis%'
           AND lower(ass.sentence_match_value) NOT LIKE '%diameter of largest lymph node metastasis%'
           AND lower(ass.sentence_match_value) NOT LIKE '%n0b is defined as no radiologic or clinical evidence of locoregional lymph node metastasis.%'
           AND lower(ass.sentence_match_value) NOT LIKE '%distant metastasis (m)%'
           AND lower(ass.sentence_match_value) NOT LIKE '%distant metastasis (pm)%'
           AND lower(ass.sentence_match_value) NOT LIKE '%distant metastasis (pm)%'
           AND lower(ass.sentence_match_value) NOT LIKE '%pn1   metastasis in regional lymph node(s)%'
     ...
     , array_to_string(array(
       ...
       , array_to_string(array(
           ...
           AND asg2.system_rejected = false
           AND asg.name = 'Metastatic Cancer'
         ), '|') AS has_cancer_site_suggestion_sentences
     ...
     , array_to_string(array(
         ...
         FROM abstractor_abstractions aa2 JOIN abstractor_suggestions asg2 on aa2.id = asg2.abstractor_abstraction_id
                                          ...
                                          JOIN abstractor_subject_groups asg ON aag.abstractor_subject_group_id = asg.id
         WHERE aa2.about_id = pivoted_abstractions.about_id
         AND aas2.predicate = 'has_metastatic_cancer_primary_site'
         AND asg2.suggested_value IS NOT NULL
         AND asg2.system_rejected = false
         AND asg.name = 'Metastatic Cancer'
       ), ', ') AS has_metastatic_cancer_primary_site_suggestions
       , array_to_string(array(
           SELECT DISTINCT ass.sentence_match_value
           FROM abstractor_abstractions aa2 JOIN abstractor_suggestions asg2 on aa2.id = asg2.abstractor_abstraction_id
                                            JOIN abstractor_subjects asb2 ON aa2.abstractor_subject_id = asb2.id
           ...
           AND asg2.suggested_value IS NOT NULL
           AND asg.name = 'Metastatic Cancer'
         ), '|') AS has_metastatic_cancer_primary_site_suggestion_sentences
...
--inside
INSERT INTO metastatic_pathology_cases
(
  abstractor_namespace_name
, west_mrn
...
SELECT
      ...
      , cancer_diagnosis_abstractions.has_cancer_histology_suggestion_match_values
      ...
      , cancer_diagnosis_abstractions.has_cancer_site_suggestion_sentences

      , cancer_diagnosis_abstractions.has_metastatic_cancer_primary_site
      , cancer_diagnosis_abstractions.has_metastatic_cancer_primary_site_suggestions
      , cancer_diagnosis_abstractions.has_metastatic_cancer_primary_site_suggestion_sentences
...
UPDATE metastatic_pathology_cases
SET has_cancer_histology = 'not applicable'
where has_cancer_histology_suggestion_sentences = ''
