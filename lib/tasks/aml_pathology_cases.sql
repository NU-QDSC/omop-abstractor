     ...
     LEFT JOIN (SELECT note_stable_identifier.id AS subject_id,
                ...
                FROM   (SELECT aas.predicate,
                        ...
                        WHERE  asb.subject_type = 'NoteStableIdentifier'
                               ...
                               AND asb.namespace_id IN(SELECT id FROM abstractor_namespaces WHERE name IN('Outside Diagnostic Pathology'))
                      ...
                      JOIN abstractor_subjects sub
                        ON aa.abstractor_subject_id = sub.id
                           AND sub.namespace_type =
                               'Abstractor::AbstractorNamespace'
                           AND sub.namespace_id IN(SELECT id FROM abstractor_namespaces WHERE name IN('Outside Diagnostic Pathology'))
      ...
      JOIN (SELECT note_stable_identifier.id AS subject_id,
            ...
            FROM   (SELECT aas.predicate,
                    ...
                    WHERE  asb.subject_type = 'NoteStableIdentifier'
                           AND asb.namespace_type =
                               'Abstractor::AbstractorNamespace'
                           AND asb.namespace_id IN(SELECT id FROM abstractor_namespaces WHERE name IN('Diagnostic Pathology', 'Outside Diagnostic Pathology'))
                ...
                FROM   abstractor_abstractions aa
                       JOIN abstractor_subjects sub
                         ON aa.abstractor_subject_id = sub.id
                            AND sub.namespace_type =
                                'Abstractor::AbstractorNamespace'
                            AND sub.namespace_id IN(SELECT id FROM abstractor_namespaces WHERE name IN('Diagnostic Pathology', 'Outside Diagnostic Pathology'))
                WHERE  aa.deleted_at IS NULL
                       AND aa.about_type = 'NoteStableIdentifier'
                       AND note_stable_identifier.id = aa.about_id)
...
--inside
INSERT INTO aml_pathology_cases
(
  abstractor_namespace_name
, west_mrn
...
from note_stable_identifier nsi
       ...
       LEFT JOIN cancer_abstractions ON data.note_id = cancer_abstractions.note_id
WHERE cancer_diagnosis_abstractions.abstractor_namespace_name = 'Diagnostic Pathology';
...
WHERE cancer_diagnosis_abstractions.abstractor_namespace_name = 'Outside Diagnostic Pathology'
;